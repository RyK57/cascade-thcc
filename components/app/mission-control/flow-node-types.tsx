...
function WalletNode({ data }: NodeProps) {
  ...
  return (
    <motion.div
      ...
      className={`w-52 rounded-xl border-2 bg-card p-3 shadow-sm ${STATE_RING[nodeData.state]}`}
    >
...
function StageNode({ data }: NodeProps) {
  ...
  return (
    <div
      className={`rounded-full border-2 bg-card px-3.5 py-1.5 text-xs font-medium ${stateStyles[nodeData.state]}`}
    >
      <Handle type="target" position={Position.Left} />
      {nodeData.title}
      {nodeData.state === "done" ? " ✓" : ""}
